...
1. **Sync** (`src/lib/sync.ts`, `src/commands/sync.ts`) — copies files between `~/.claude/` and the `.claude-sync/` git repo, driven by the `FILE_MAPPINGS` list.
   ...
   - `meta.json` (`createMetaJson`/`readMetaJson`) stores repository identity, machine id, platform, and the portable Claude config path inside `.claude-sync/`. Legacy `lastSync` fields are removed on the next push to avoid metadata-only Git churn.

2. **Profiles** (`src/lib/profiles.ts`, `src/commands/profile.ts`) — lets several Claude configs coexist. A profile lives in `~/.claude-<name>/` and **symlinks** the `SHARED_ITEMS` (settings.json, hooks/, agents/, skills/, commands/, scripts/, plugins/, keybindings.json) back to `~/.claude/`, so edits to the main config propagate instantly. A `claude-<name>` shell alias (written into `.zshrc`/`.bashrc`) launches Claude Code with `CLAUDE_CONFIG_DIR` pointed at the profile dir. `profiles.json` (in `.claude-sync/`) is the registry.
