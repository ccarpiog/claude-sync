...
export async function handleSyncPush(): Promise<void> {
  ...
  if (!(await isGitRepo(claudeSyncDir))) {
    throw new ClaudeSyncError(
      `${formatPath(claudeSyncDir)} is not a Git repository`,
      ErrorCode.NOT_GIT_REPO,
      'Run "claude-sync sync setup" to configure syncing.'
    );
  }
...
export async function handleSyncPull(options: { force?: boolean } = {}): Promise<void> {
  ...
  if (!gitStatus.remote) {
    throw new ClaudeSyncError(
      ...
      ErrorCode.NO_REMOTE,
      'Run "claude-sync sync setup" to set up a remote repository.'
    );
  }
  ...
  if (await hasMergeConflicts(claudeSyncDir)) {
    throw new ClaudeSyncError(
      ...
      `Resolve conflicts in ${formatPath(claudeSyncDir)} and run pull again.`
    );
  }
