 ...
 */
async function directoriesInSync(
  sourcePath: string,
  targetPath: string
): Promise<boolean> {
...
export async function compareFiles(
  sourceDir: string,
  targetDir: string
): Promise<Array<{ mapping: FileMapping; inSync: boolean; sourceExists: boolean; targetExists: boolean }>> {
  return Promise.all(
    FILE_MAPPINGS.map(async (mapping) => {
...
export async function removeLegacyLastSync(claudeSyncDir: string): Promise<boolean> {
  const metaPath = path.join(claudeSyncDir, 'meta.json');
  if (!fs.existsSync(metaPath)) {
    return false;
  }
  ...
  try {
    const meta = await fs.readJson(metaPath) as Record<string, unknown>;
